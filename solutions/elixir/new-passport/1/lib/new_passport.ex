defmodule NewPassport do
  @doc """
  I've also written an implementation without the when keyword below. I did that
  first to grasp the task before understand how the when keyword changes things.
  """
  @spec get_new_passport(NaiveDateTime.t(), Date.t(), atom()) :: String.t()
  ...
  # def get_new_passport(now, birthday, form) do
  #   {status, timestamp} = enter_building(now)

  #   if status === :error do
  #     {status, timestamp}
  #   else
  #     {status, manual} = find_counter_information(now)

  #     if status === :coffee_break do
  #       {:retry, NaiveDateTime.add(now, 15, :minute)}
  #     else
  #       counter = manual.(birthday)
  #       {status, checksum} = stamp_form(timestamp, counter, form)
