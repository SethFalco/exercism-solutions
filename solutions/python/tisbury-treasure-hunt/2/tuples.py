...
def get_coordinate(record: tuple[str, str]) -> str:
    ...
    :param record: with a (treasure, coordinate) pair.
...
def create_record(azara_record: tuple, rui_record: tuple[str, tuple, str]) -> tuple | str:
    """
...
def clean_up(combined_record_group: tuple[tuple[str, str, str, tuple[str, str], str]]) -> str:
    ...
    """
    transformed = [str((treasure, loc, coord, quad)) for treasure, _, loc, coord, quad in combined_record_group]
    return "\n".join(transformed) + "\n"
